...
{
    auto SQUARE = [](Double_t x){return x * x;};
...
std::pair<TF1&, TF1&>
    ...
    std::vector<Double_t> lInitialPars({
        lXYmax, // 0 mean gauss 
         .1,    // 1 sig gauss 
        ...
        2.,     // 5 n1 rhs
        lYmax});// 6 amplitude 
    f_BefNorm.SetParameters(theInitialPars ? 
                            theInitialPars : lInitialPars.data());
...
TPairFitsWAxis& 
    ...
    std::vector<TH1*> &vHists_ptG_i_dn_dr 
        = *new std::vector<TH1*>(ptBinMax+1, static_cast<TH1*>(nullptr));
    std::vector<TF1*> &vFits_ptG_i_dp_dr 
        = *new std::vector<TF1*>(ptBinMax+1, static_cast<TF1*>(nullptr));
    ...
    for (int i=binStart; i<=ptBinMax; ++i){ cout << i << endl;
