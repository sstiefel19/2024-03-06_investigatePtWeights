...
class Integrand_dN_dptR__FO {
    ...
    double operator() (double *x, double *p) const {
        ...
        if (r > -1. && r < 1. ){ 
            ...
            auto f_dp_dr = inRange ?  
                vFits_ptG_i_dp_dr.at(iPtGBin) : static_cast<TF1*>(nullptr);
...
class  MyWeightedDistFunctionObject {
    public:
        MyWeightedDistFunctionObject(TF1 *_dataF, TH1 *_mcH, TF1 *theDistToBeWeighted) : 
            fDataDist(_dataF), 
            fSampledMCHist(_mcH), 
...
class MCEffi {
    public:
        MCEffi(std::string _id, TH1* _hGen_i, TF1* _fD_i, TF1* _trueEffi):
            fID(_id),
        ...
        void PlotAll(TLegend *theLeg=nullptr){
            for(TObject *obj: *fListAllInit){
                ...
                obj->Draw("same");
                if (theLeg){theLeg->AddEntry(obj, obj->GetName());}
            }
        }
