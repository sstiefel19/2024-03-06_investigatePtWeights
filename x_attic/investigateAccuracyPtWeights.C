...
TF1* getMesonEfficiency(std::string fname){
    ...
    auto drawAndAddToLegendF = [leg2](TF1* f, EColor c=kBlue){f->SetLineColor(c); f->Draw("same"); leg2->AddEntry(f,f->GetName());};
...
void investigateAccuracyPtWeights(){
    ...
    TH1* hMC_inv = (TH1*)getObjectFromPathInFile(infile, histoname, histoname+"_clone");
    TH1* hMC_invA = (TH1*)getObjectFromPathInFile(infile, histonameA, histoname+"_cloneA");
    infile.Close();
    ...
    auto drawAndAddToLegend = [leg2](TH1* h){reduceErrorsByFactor(*h); h->Draw("same"); leg2->AddEntry(h,h->GetName());};
    auto drawAndAddToLegendF = [leg2](TF1* f){f->Draw("same"); leg2->AddEntry(f,f->GetName());};
    
    //~ TF1* fEffi_d2 = new TF1("fEffi_d2",[&](double*x, double *p){ return myProviEffi->Derivative2(*x); }, 0, lXmax, 0);
    //~ drawAndAddToLegendF(fEffi_d2);
    

    TH1* hEffiWW_ASoverMB = divideTH1ByTH1(*lMCAS.GetMeasuredEffiWW(), *lMCMB.GetMeasuredEffiWW(), "", "hEffiWW_ASoverMB");
    drawAndAddToLegend(hEffiWW_ASoverMB);
    ...
    drawAndAddToLegend(hASGenWWoverMBGenWW);
